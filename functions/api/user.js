...
export async function onRequest({ request, env }) {
  try {
    ...
    // 固定旧域名兜底 blog.lizhuoxuan.dpdns.org
    ...
    if (action === "microsoftLogin") {
      const msCid = env?.MS_CLIENT_ID;
      const redirectUri = `${siteOrigin}/api/microsoftCallback`;
      if (!msCid) return jsonResp({ code: 500, msg: "微软登录客户端ID未配置" }, 500);
      const state = await generateOauthState(hmacSecret, "login");
    ...
    if (action === "microsoftBind") {
      if (!loginUser) return jsonResp({ code: 401, msg: "请登录后再绑定" }, 401);
      const msCid = env?.MS_CLIENT_ID;
      const redirectUri = `${siteOrigin}/api/microsoftCallback`;
      const state = await generateOauthState(hmacSecret, "bind");
      const authUrl = new URL("https://login.microsoftonline.com/common/oauth2/v2.0/authorize");
    ...
    // 移除堆栈泄露，生产环境不返回底层错误
    console.error("Worker全局异常：", globalErr.message);
