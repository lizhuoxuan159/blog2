...
export async function onRequest({ request, env }) {
  try {
    ...
    if (action === 'check') {
      ...
      const userInfoRaw = await db.prepare(`SELECT role, is_cancel, github_id, microsoft_id FROM users WHERE id = ?`).bind(loginUser.uid ?? null).first();
      if (!userInfoRaw || userInfoRaw.role === "banned" || userInfoRaw.is_cancel === 1) {
      ...
      // null 全部转为空串，前端绝对不会 undefined
      const userInfo = {
        role: userInfoRaw.role || 'guest',
        is_cancel: userInfoRaw.is_cancel ?? 0,
        github_id: userInfoRaw.github_id || '',
        microsoft_id: userInfoRaw.microsoft_id || ''
      };
      return jsonResp({
        login: true,
        uid: loginUser.uid,
    ...
    if (action === "sendEmailCode") {
      ...
      if(reqMethod === "GET") return jsonResp({code:0,msg:"接口正常，请POST提交参数"});
      if(reqMethod !== "POST") return jsonResp({ code: 405, msg: "请求方式非法，请使用POST提交" },405);
      const {email} = await request.json();
      return jsonResp({code:0,msg:"验证码发送接口已就绪"});
    }
    ...
    if (action === 'userList' && reqMethod === 'GET') {
      ...
      // 循环兜底 null 转空串，前端渲染绝对不出 undefined
