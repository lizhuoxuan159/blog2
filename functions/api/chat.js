    ...
    try {
      ...
      `).bind(msgId).all();
            const comments = rawComments.results.map(item => {
                let displayName = item.username;
                if (item.is_cancel === 1 || item.username === null) displayName = "账户已注销";
                return {
                    id: item.id,
                    content: item.content,
      ...
      `).bind(loginUser.uid).first();
            ...
            const msg = await db.prepare(`SELECT id FROM messages WHERE id = ?`).bind(msgId).first();
            if (!msg) return jsonResp({ code: 1, msg: '消息不存在' });
            await db.prepare(`
